...
| `contextMenus` | Adds "Ask AI Skope about this" to the right-click menu on selected text. Off by default: a fresh install leaves the page's own menu untouched, and the entry appears only while the user has switched it on in the pane's settings. |
